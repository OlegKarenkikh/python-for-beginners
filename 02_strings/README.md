...
- Создать строку в одинарных и двойных кавычках
- Применять методы: `.upper()`, `.lower()`, `.strip()`, `.split()`, `.replace()`
- Форматировать вывод через f-строки: `f"Клиент: {name}"`
- Объяснить почему строки неизменяемы и что значит «сохранить результат»
