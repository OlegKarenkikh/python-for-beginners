...
- Перебрать список через `for`
- Использовать `range()` для числовых циклов
- Остановить цикл через `break`, пропустить итерацию через `continue`
- Написать `while` с условием выхода
- Применять `enumerate()` и `zip()` для параллельного обхода
