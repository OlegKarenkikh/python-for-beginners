...
```python
TOOL_REGISTRY = {
    "calculate_premium": calculate_premium,  # наклейка "calculate_premium" → прибор Чайник
    "find_client":       find_client,        # наклейка "find_client"        → прибор Радио
    "check_documents":   check_documents,    # наклейка "check_documents"    → прибор Лампа
}
```
